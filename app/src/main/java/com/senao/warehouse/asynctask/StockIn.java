...
public class StockIn extends AsyncTask<BasicHelper, Void, BasicHelper> {
    private AsyncResponse<BasicHelper> mCallBack;
    private WeakReference<Activity> mWeakActivity;
    private ProgressDialog mProgressDialog;
    private StockInHandler mHandler;

    public StockIn(Activity activity, AsyncResponse<BasicHelper> callback) {
    ...
    protected void onPreExecute() {
        ...
        if (activity != null) {
            mProgressDialog = ProgressDialog.show(activity, activity.getString(R.string.datathrownerp), activity.getString(R.string.holdon), true);
        }
    }

    @Override
    protected BasicHelper doInBackground(BasicHelper... params) {
        return mHandler.doStockIn(params[0]);
    }
