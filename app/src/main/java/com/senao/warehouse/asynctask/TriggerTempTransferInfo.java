...
public class TriggerTempTransferInfo extends AsyncTask<BasicHelper, Void, BasicHelper> {
    private AsyncResponse<BasicHelper> mCallBack;
    private WeakReference<Activity> mWeakActivity;
    private ProgressDialog mProgressDialog;
    private TransferHandler mTransferHandler;

    public TriggerTempTransferInfo(Activity activity, AsyncResponse<BasicHelper> callback) {
    ...
    protected void onPreExecute() {
        ...
        if (activity != null) {
            mProgressDialog = ProgressDialog.show(activity, activity.getString(R.string.processing), activity.getString(R.string.holdon), true);
        }
    }

    @Override
    protected BasicHelper doInBackground(BasicHelper... params) {
        return mTransferHandler.triggerTempTransferInfo(params[0]);
    }
