...
public class DeleteTempTransferInfo extends AsyncTask<BasicHelper, Void, BasicHelper> {
    private AsyncResponse<BasicHelper> mCallBack;
    private WeakReference<Activity> mWeakActivity;
    private ProgressDialog mProgressDialog;
    private TransferHandler mHandler;

    public DeleteTempTransferInfo(Activity activity, AsyncResponse<BasicHelper> callback) {
    ...
    protected void onPreExecute() {
        ...
        if (activity != null) {
            mProgressDialog = ProgressDialog.show(activity, activity.getString(R.string.del_data), activity.getString(R.string.holdon), true);
        }
    }

    @Override
    protected BasicHelper doInBackground(BasicHelper... params) {
        return mHandler.delTempTransferInfo(params[0]);
    }
