...
public class GetEzFlowDocumentListForRevoke extends AsyncTask<BasicHelper, Void, BasicHelper> {
    private AsyncResponse<BasicHelper> mCallBack;
    private WeakReference<Activity> mWeakActivity;
    private ProgressDialog mProgressDialog;
    private TransferHandler mTransferHandler;

    public GetEzFlowDocumentListForRevoke(Activity activity, AsyncResponse<BasicHelper> callback) {
    ...
    protected void onPreExecute() {
        ...
        if (activity != null) {
            mProgressDialog = ProgressDialog.show(activity, activity.getString(R.string.get_data), activity.getString(R.string.holdon), true);
        }
    }

    @Override
    protected BasicHelper doInBackground(BasicHelper... params) {
        return mTransferHandler.getEzFlowDocumentListForRevoke(params[0]);
    }
