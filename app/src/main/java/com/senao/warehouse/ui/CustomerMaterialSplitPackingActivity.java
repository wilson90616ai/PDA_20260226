...
public class CustomerMaterialSplitPackingActivity extends Activity {
    private static final int REQUEST_ENABLE_BT = 1;
    private static final int REQUEST_PRINTER_SETTING = 2;
    private static final int REQUEST_BLUETOOTH_SETTINGS = 3;
    private static final String TAG = CustomerMaterialSplitPackingActivity.class.getSimpleName();
    private TextView mConnection;
    private EditText txtImportPartNo, txtImportPartNoCustomer, txtImportDC, txtImportVC, txtImportOriginalQty,
            txtImportSplitQty, txtImportPo, txtImportQrCode, txtImportReelId, txtImportReelIdOriginalQty,
            txtImportReelIdSplitQty, txtImportReelIdPo, txtImportQrCodeSplitQty;
    ...
    private RadioGroup rgDC, rgPrinter, rgPnType;
    private RadioButton rbQrCode, rbReelID, rbPartNo, rbBluetooth, rbWifi, rbTypePn, rbTypeCustomerPn;
    private LinearLayout qrcodeLayout, pnLayout, reelIdLayout;
    private TextView lblTitle;
    private String mCustomerPartNo;
    private PnItemInfoListHelper pnInfoList;
    ...
    private RadioGroup.OnCheckedChangeListener pnTypeListener = new RadioGroup.OnCheckedChangeListener() {

        @Override
        public void onCheckedChanged(RadioGroup group, int checkedId) {
            switch (checkedId) {
                case R.id.radio_type_pn:
                    if (rbTypePn.isChecked()) {
                        txtImportPartNoCustomer.setText("");
                        txtImportPartNoCustomer.setVisibility(View.GONE);
                        txtImportPartNo.setVisibility(View.VISIBLE);
                        txtImportPartNo.requestFocus();
                    }
                    break;
                case R.id.radio_type_cpn:
                    if (rbTypeCustomerPn.isChecked()) {
                        txtImportPartNo.setText("");
                        txtImportPartNo.setVisibility(View.GONE);
                        txtImportPartNoCustomer.setVisibility(View.VISIBLE);
                        txtImportPartNoCustomer.requestFocus();
                    }
                    break;
            }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_customer_material_split_packing);
        ...
        mConnection.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!TextUtils.isEmpty(errorInfo)) {
                    AlertDialog.Builder dialog = new AlertDialog.Builder(CustomerMaterialSplitPackingActivity.this);
        ...
        String subtitle = getString(R.string.label_customer_material_split_packing_eng);
        String title = getString(R.string.label_customer_material_split_packing1,AppController.getOrgName(), subtitle);
        text = new SpannableString(title);
        text.setSpan(new RelativeSizeSpan(1.0f), 0, title.length() - subtitle.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        text.setSpan(new RelativeSizeSpan(0.7f), title.length() - subtitle.length(), title.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        rbTypePn = findViewById(R.id.radio_type_pn);
        rbTypeCustomerPn = findViewById(R.id.radio_type_cpn);

        rgPnType = findViewById(R.id.radio_part_no_type);
        rgPnType.setOnCheckedChangeListener(pnTypeListener);
        ...
        txtImportPartNo.setFilters(new InputFilter[]{new InputFilter.LengthFilter(12)}); //限制長度為12
        txtImportPartNo.setSelectAllOnFocus(true);

        txtImportPartNoCustomer = findViewById(R.id.edittext_pn_customer);
        txtImportPartNoCustomer.setSelectAllOnFocus(true);
    ...
    private void onCreateChoosePnSelectDialog() {
        String[] pnList = new String[pnInfoList.getPnList().length];

        for (int i = 0; i < pnInfoList.getPnList().length; i++) {
            pnList[i] = pnInfoList.getPnList()[i].getPartNo() + " " + pnInfoList.getPnList()[i].getPartDescription();
        }

        AlertDialog.Builder dialog_list = new AlertDialog.Builder(this);
        dialog_list.setCancelable(false);
        dialog_list.setTitle(R.string.choose_sku);
        dialog_list.setItems(pnList, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                pnInfo = pnInfoList.getPnList()[which];

                if (rbPartNo.isChecked()) {
                    txtImportPartNo.setText(pnInfo.getPartNo());
                    txtImportPartNoCustomer.setText(pnInfo.getCustomerPartNo());
                }

                mCustomerPartNo = pnInfo.getCustomerPartNo();
        ...
        dialog_list.setNegativeButton(R.string.choose_sku, new DialogInterface.OnClickListener() {
    ...
    private void doQueryPN() {
        ...
        if (rbPartNo.isChecked()) {
            if (rbTypePn.isChecked()) {
                ...
                pnInfo.setCustomerPartNo(getCustomerPartNo());
            }
        } else {
            pnInfo.setPartNo(getPartNo());
        }
        ...
        new GetPNInfoFromSfis().execute(0);
    }
        ...
        if (pnLayout.getVisibility() == View.VISIBLE) {
            ...
            if (rbTypePn.isChecked()) {
                if (TextUtils.isEmpty(txtImportPartNo.getText())) {
                    txtImportPartNo.requestFocus();
                    Toast.makeText(getApplicationContext(), getString(R.string.enter_senao_sku), Toast.LENGTH_SHORT).show();
                    return false;
                }
                ...
                if (TextUtils.isEmpty(txtImportPartNoCustomer.getText())) {
                    txtImportPartNoCustomer.requestFocus();
                    Toast.makeText(getApplicationContext(), getString(R.string.enter_cust_sku), Toast.LENGTH_SHORT).show();
                    return false;
                }
            }
            ...
            if (Integer.parseInt(txtImportSplitQty.getText().toString())
                    > Integer.parseInt(txtImportOriginalQty.getText().toString())) {
                txtImportSplitQty.requestFocus();
    ...
    private Dialog onCreateChoosePrinterDialog() {
        ...
        String ip = Preferences.getSharedPreferences(this).getString(Preferences.CUSTOMER_MATERIAL_PRINTER_IP, AppController.getProperties("CustomerMaterialPrinterIP"));
        etIP.setText(ip);
        final EditText etPort = item.findViewById(R.id.editTextPort);
        String port = Preferences.getSharedPreferences(this).getString(Preferences.CUSTOMER_MATERIAL_PRINTER_PORT, AppController.getProperties("CustomerMaterialPort"));
        etPort.setText(port);
        final EditText etQty = item.findViewById(R.id.editTextQty);
        String qty = Preferences.getSharedPreferences(this).getString(Preferences.CUSTOMER_MATERIAL_PRINTER_QTY, AppController.getProperties("CustomerMaterialPrinterQty"));
        etQty.setText(qty);
                ...
                button.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (savePrinterInfo(etIP.getText().toString().trim(), etPort.getText().toString().trim(), etQty.getText().toString().trim())) {
                            alertDialog.dismiss();
                            dialog = ProgressDialog.show(CustomerMaterialSplitPackingActivity.this, getString(R.string.holdon), getString(R.string.printingLabel), true);
                            ...
                            for (int j = 0; j < printTimes; j++) {
                                ...
                                if (!printLabel(reelId, getPartNo(), pnInfo.getPartDescription(), String.valueOf(newQty)
                                        , getDateCode(), getVendorCode(), po, qrCode, getCustomerPartNo())) {
                                    printClose();
                                    Toast.makeText(getApplicationContext(), "列印新標籤出現問題，請重新列印", Toast.LENGTH_LONG).show();
                                    dialog.dismiss();
                                    return;
                                }
                                ...
                                if (!printLabel(subMlInfo.getReelIds()[0], getPartNo(), pnInfo.getPartDescription(), String.valueOf(splitQty)
                                        , getDateCode(), getVendorCode(), po, qrCode, getCustomerPartNo())) {
                                    printClose();
    ...
    private void printLabel() {
        ...
        if (!BtPrintLabel.printCustomerMaterialLabel1(reelId, getPartNo(), pnInfo.getPartDescription(), String.valueOf(newQty), getDateCode(), getVendorCode(), po, qrCode, getCustomerPartNo(),orgPrint)) {
            Toast.makeText(getApplicationContext(), getString(R.string.reprinting_due_to_issue_with_sub_package_label), Toast.LENGTH_LONG).show();
            dialog.dismiss();
            return;
        }
        ...
        if (!BtPrintLabel.printCustomerMaterialLabel1(subMlInfo.getReelIds()[0], getPartNo(), pnInfo.getPartDescription(), String.valueOf(splitQty), getDateCode(), getVendorCode(), po, qrCode, getCustomerPartNo(),orgPrint)) {
            Toast.makeText(getApplicationContext(), getString(R.string.reprinting_due_to_issue_with_sub_package_label), Toast.LENGTH_LONG).show();
            dialog.dismiss();
            return;
        }
    ...
    public String getCustomerPartNo() {
        if (pnLayout.getVisibility() == View.VISIBLE) {
            return txtImportPartNoCustomer.getText().toString().trim();
        } else {
            return mCustomerPartNo;
        }
    }
    ...
    private void printTest() {
        ...
        String customerPartNo = "BVTBKF21EAW";

        if (!printLabel(reelId, partNo, description, qty, dateCode, vendorCode, po, qrCode, customerPartNo)) {
    ...
    private boolean printLabel(String reelId, String partNo, String Description, String qty,
        ...
        try {
            ...
            if (!TextUtils.isEmpty(customerPartNo)) {
                x = mm2dot(1);
                y = mm2dot(43);
                TscEthernetDll.printerfont(x, y, "0", 0, 7, 7, "Customer P/N:");
                x = mm2dot(19);
                y = mm2dot(41);
                TscEthernetDll.barcode(x, y, "128", 30, 0, 0, 2, 2, customerPartNo);
                x = mm2dot(19);
                y = mm2dot(44);
                TscEthernetDll.printerfont(x, y, "0", 0, 12, 12, customerPartNo);
            }
    ...
    private boolean savePrinterInfo(String ip, String port, String qty) {
        try {
            Preferences.setString(this, Preferences.CUSTOMER_MATERIAL_PRINTER_IP, ip);
            Preferences.setString(this, Preferences.CUSTOMER_MATERIAL_PRINTER_PORT, port);
            Preferences.setString(this, Preferences.CUSTOMER_MATERIAL_PRINTER_QTY, qty);
    ...
    private class GetPNInfoFromSfis extends AsyncTask<Integer, String, PnItemInfoListHelper> {
        @Override
        protected PnItemInfoListHelper doInBackground(Integer... params) {
            AppController.debug("Get PNInfo from " + AppController.getServerInfo() + AppController.getProperties("GetPNInfoSfis"));
            publishProgress(getString(R.string.downloading_data));
            return print.getPNInfoFromSfis(pnInfo);
        }
        ...
        @Override
        protected void onPostExecute(PnItemInfoListHelper result) {
            ...
            if (result != null) {
                pnInfoList = result;
                if (pnInfoList.getIntRetCode() == ReturnCode.OK) {
                    mConnection.setText("");
                    mConnection.setTextColor(Color.WHITE);
                    mConnection.setBackgroundColor(Color.rgb(164, 199, 57));
                    errorInfo = "";

                    if (pnInfoList.getPnList() == null || pnInfoList.getPnList().length == 0) {
                        Toast.makeText(getApplicationContext(), getString(R.string.cant_find_sku), Toast.LENGTH_LONG).show();
                    } else {
                        if (pnInfoList.getPnList().length == 1) {
                            pnInfo = pnInfoList.getPnList()[0];
                    ...
                    errorInfo = pnInfoList.getStrErrorBuf();
                    mConnection.setTextColor(Color.RED);
                    mConnection.setBackgroundColor(Color.rgb(164, 199, 57));
                }
            } else {
    ...
    private class GetReelIdList extends AsyncTask<Integer, String, MaterialLabelHelper> {
        ...
        protected MaterialLabelHelper doInBackground(Integer... params) {
            AppController.debug("Get Reel ID List from " + AppController.getServerInfo() + AppController.getProperties("GetReelIdList"));
            publishProgress(getString(R.string.downloading_data));
    ...
    private class GetReelIdSubList extends AsyncTask<Integer, String, MaterialLabelHelper> {
        @Override
        protected MaterialLabelHelper doInBackground(Integer... params) {
            AppController.debug("Get Reel ID Sub List from " + AppController.getServerInfo() + AppController.getProperties("GetReelIdSubList"));
            publishProgress(getString(R.string.downloading_data));
